...
/// Tela dedicada a Serviços: listagem, cadastro e edição.
...
class _ServicosPageState extends State<ServicosPage> {
  ...
  Future<void> _deleteServico(Map<String, dynamic> item) async {
    final id = item['id'];
    if (id == null) return;
    final idInt = id is int ? id : int.tryParse(id.toString());
    if (idInt == null || idInt <= 0) return;

    final desc = item['descricao']?.toString() ?? 'este serviço';
    ...
    try {
      await ApiClient.deleteServico(idInt);
      if (!mounted) return;
      _load();
  ...
  List<Map<String, dynamic>> get _filtered {
    if (_searchQuery.isEmpty) return _servicosList;
    final q = _searchQuery.toLowerCase();
    return _servicosList
        .where((s) => (s['descricao']?.toString() ?? '').toLowerCase().contains(q))
        .toList();
  }
      ...
      child: Column(
        ...
        children: [
          ...
          ElevatedButton.icon(
            ...
            label: const Text('Cadastrar serviço', style: TextStyle(color: Colors.white)),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            'Nenhum serviço encontrado para "$_searchQuery".',
...
class _ServicoCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final ativo = item['ativo'] as bool? ?? true;
    final icone = iconFromCode(item['icone']?.toString()) ?? Icons.content_cut;
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(
              icone,
              color: ativo ? AppColors.loginOrange : AppColors.loginTextMuted,
              size: 28,
            ),
          ),
                  ...
                  children: [
                    ...
                    if (!ativo) ...[
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: AppColors.loginTextMuted.withOpacity(0.3),
