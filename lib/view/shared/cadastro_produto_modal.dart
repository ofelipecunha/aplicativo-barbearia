...
/// Modal de cadastro/edição de Produto. [item] null = criar; preenchido = editar.
void showCadastroProdutoModal(
  ...
  showDialog(
    context: context,
    builder: (ctx) => _CadastroProdutoDialog(
      item: item,
      onSuccess: onSuccess,
    ),
  );
...
class _CadastroProdutoDialog extends StatefulWidget {
  final Map<String, dynamic>? item;
  final VoidCallback onSuccess;

  const _CadastroProdutoDialog({
    this.item,
    required this.onSuccess,
  });

  @override
  State<_CadastroProdutoDialog> createState() => _CadastroProdutoDialogState();
}

class _CadastroProdutoDialogState extends State<_CadastroProdutoDialog> {
  ...
  void initState() {
    ...
    if (i != null) {
      ...
      _valorVendaCtrl.text = _brl((i['valor_venda'] as num?)?.toDouble() ?? 0);
      final est = (i['estoque'] as num?)?.toInt();
      _estoqueCtrl.text = (est != null && est > 0) ? est.toString() : '';
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              _field('Valor de venda (R\$)', _valorVendaCtrl, hint: 'Ex: 45,00'),
              const SizedBox(height: 12),
              _field('Estoque', _estoqueCtrl,
                  hint: 'Ex: 10',
...
/// Remove zeros à esquerda ao digitar (ex.: "05" → "5").
class _StripLeadingZerosFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(
    ...
    if (n == null) return newValue;
    final s = n.toString();
    if (s == newValue.text) return newValue;
